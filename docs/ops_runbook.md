...
- chat renderer and immutable session binding: `src/gp_assistant/chat_agent.py` + `src/gp_assistant/agent_store.py`
- market worker: `src/gp_assistant/worker.py`
...
- Serenity resident native-Alpha service: `src/gp_assistant/serenity/` and `decision_engine/serenity_policy.py`
- session and transcript storage: `src/gp_assistant/agent_store.py`
...
- preselection publishes an immutable candidate target; Serenity freezes exact-target as-of Alpha; Adaptive performs one nine-expert final score
- Market Memory similarity retrieval uses normalized feature-vector distance, not label-only `signal_type` matching
- local code fixes decision/action/numbers; the LLM performs one TurnFrame routing call and one grounded narration call, neither with selection authority
...
The ordinary resident loop owns bootstrap, live polling, retry checkpoints and target switches. Do not manufacture readiness with a manual one-shot bootstrap or injected data.
...
The resident service runs in `native` mode. A successful complete zero-result official query is valid neutral Alpha and must report `no_relevant_evidence`; incomplete/unparsed/stale coverage is unavailable and blocks the exact candidate target. `off` makes production fail closed. Legacy `auto` and `reference` values are rejected.
...
docker compose up -d
docker ps
docker compose logs --tail=80 api worker serenity web
...
```bash
docker compose exec -T api python -c "import gp_assistant.decision_engine.pipeline as p; print(p.run_market_memory_selection.__name__)"
```
...
- a committed turn must record real `intent_routing` (or repair) followed by `tool_evidence`, each with 2xx/model/response ID
...
- `store/portfolio/`
- `store/validation/`
- `store/serenity/` (ignored append-only resident evidence DB and raw PDFs)
...
- `agent.db` and the three public endpoints are the sole product-facing boundary; removed recommendation/workbench surfaces are not repair tools.

## When Something Looks Wrong

1. Check `/api/health`, especially `product_ready`, `readiness_reasons`, `worker.runtime_contract_ready`, `serenity.worker_alive`, exact target coverage, and `llm.verification`.
...
6. Inspect the current `RecommendationSnapshot.v1` in `agent.db`; legacy `store/book/current.json` is not product authority.
...
10. For Serenity, inspect the top-level `serenity` health block. `bootstrap_ready=false`, `worker_alive=false`, `stale=true`, incomplete target coverage, unparsed content or an open breaker means the native candidate target cannot publish; do not infer “no bad news.”
## Rebuild and repair contract

Build the backend once with `docker compose build api`; every Python service references the resulting `gp-backend` tag. Start all resident services with `docker compose up -d`.

After a rebuild, let the resident worker and Serenity service advance through their ordinary contracts. Verify `product_ready/readiness_reasons`, exact target lineage, worker lease, current market-time snapshot, and then a real two-stage `/api/chat` turn. A liveness-only Compose health result is not product acceptance.
