...
当前推荐系统定位：日 K 为主的下一交易窗口计划。Market-Memory 推荐使用已完成日线、历史相似事件、概率和风险证据；盘中如果没有可靠实时执行数据，系统必须明确说明只能提供日线计划状态。

Historical Replay 必须冻结历史日期 `T` 可见的数据：信号构建使用 `as_of=T` 的日线，Market Memory 只检索 `event.as_of < T` 且 outcome 在 `T` 前已可知的事件，概率校准和未来 T+1/T+3/T+5 验证不得在推荐生成前进入上下文。
