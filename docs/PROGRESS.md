...
- **Active Goal:** Restore a dated, immutable and publish-blocking full-market main-board universe, then keep Serenity as a bounded additive Alpha that cannot veto a valid base recommendation.
- **Active ExecPlan:** `docs/plans/2026-07-22-full-market-universe-recovery.md`.
- **2026-07-22 Full-Market Recovery Complete:** Added immutable `MarketUniverseSnapshot.v1` storage and official SSE/SZSE master normalization; production no longer permits the static symbol-file fallback. Universe `mus_20260722_12adb91c92ebb41babf6` passed with 3,191 main-board names, 3,190 target-day histories (99.97%), 606 eligible symbols, a 200-symbol scoring pool, 200 successful scores and no fallback. Current snapshot `daily_29989bdcae26`, health, MarketBook and the rendered Workspace bind the same universe. Serenity's incomplete 30-symbol batch is exposed as 0/30, effective weight 0 and does not block `product_ready=true`.
- **Current Milestone:** Recovery implementation, automated regression, Docker rebuild, source-image provenance, rendered Workspace inspection and real LLM Top-3 plus same-session follow-up acceptance are complete. The remaining ongoing work is normal forward Serenity observation, not candidate-universe recovery.

- **2026-07-22 Architecture Research Checkpoint:** A complete read-only audit found that the prior production recommendation path was not validated as a full-market/main-board product flow. Since `8cfde8b` (2026-07-09), `build_day_selection` disabled the market snapshot and fell back to a static ten-symbol file; output-subset freshness could therefore report ready without proving market-wide coverage. Serenity did not introduce that switch, but native integration was coupled into the same central pipeline and could convert incomplete extension coverage into a base `no_trade`. The evidence is recorded in `docs/architecture-audit-2026-07-22-candidate-universe-and-serenity-boundaries.md`; the repair and local backfill were subsequently authorized and are tracked by the active ExecPlan.
- **2026-07-21 Recovery:** The lunch runtime can now publish a board whose display rank follows intraday execution readiness without mutating the immutable daily decision rank inside each pick. LLM narration no longer requires opaque GPVAL tokens for every number: it may use exact supplied values when candidate/field validation succeeds, while fabricated/rounded numbers, selection or action overrides, sizing, and Serenity-boundary violations still fail closed.
- **2026-07-21 Score disclosure:** Narration now instructs the model to print each supplied candidate's own `final_score` as its comprehensive score; probability and confidence cannot stand in for that score.
- **2026-07-21 Routing authority:** Removed local user-text reconstruction of `topk`, candidate scope, references, and refresh intent. The strict LLM router now owns those semantic fields; local processing retains only validation and snapshot-bound resolution.
- **2026-07-21 Prior-set routing:** The complete previous assistant candidate list is now retained in routing context, and natural-language filtering of “这里面/上述/这批” is instructed to preserve that multi-candidate scope.
...
- 2026-07-21: At user direction, the complete post-generation LLM narration
  validation and repair layer is disabled. The response still carries the
  immutable local snapshot's structured decision/tradeability fields, but
  narration prose is no longer rejected for symbols, numbers, actions, sizing,
  or Serenity wording. Revisit only with a replacement that can reliably
  explain no-trade states without blocking a valid user reply.
- 2026-07-21: Separated morning-session freshness from the unchanged daily
  snapshot interface. Workspace uses `/api/lunch/current` for lunch status;
  `READY` means only the morning session is complete, never that today's daily
  data is complete.
