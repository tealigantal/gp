...
- [x] Native August 4 recovery, deployed page/chat and old-payload immutability verified. Source commits `142ed3f` and `67ce73a` pushed to main; both remote CI runs passed.
...
Deployment follow-up: the standard backend build stalled downloading PyArrow. Cancelled only that task-owned build client; reused verified backend image `b4db1077` as the dependency base, copied the complete current src/config/pyproject and stamped revision `142ed3f`. New image `f1212f9a` passed all 180 tests offline with disposable stores and read-only test/docs mounts. No Docker service/cache/volume reset. Web built normally. The first native retry remained scheduled at 22:20:34 after graceful worker shutdown. Real post-deploy chat exposed a missing historical-backlog fact in narrator input: added the separate public date/count/state projection and its boundary instruction, with a production temporal-function regression.

Final follow-up acceptance (22:22): native worker completed 2026-08-04 at 22:21:11.106706+08:00. 603221 is `excluded/official_suspension`, source 1225452030, policy v3, two inclusive sessions, five-session limit, trigger `investigation_disclosure`; no manual ledger writes. The historical run retains its original approximate denominator: 3044 tracked rows = 3042 fetched + one official halt + one pre-existing pre-listing exclusion. Do not describe this historical reconstructed universe as contemporaneous full-market proof. Recovery advanced to August 5; other historical backlog is not claimed complete.

Final backend image `sha256:b3beca97ea61919d8a10b71ee95806444583b2ce814211e6e00d9f426262a9d5` and web `sha256:aecfd2a472fdff9551e4742a26107fd7efe2c16181e311558e1f34c8754e6535` are deployed. Backend reused the same verified dependency base, copied full current `src/`, `configs/daily_scoring.json`, `pyproject.toml`, and set ENV/OCI revision to `67ce73a732fdb4979773c1fff64ffdb52a8a394a`. All five changed source hashes match checkout in both API and worker. Final 181 backend tests passed locally and in the offline image; 18 frontend tests, typecheck, lint/build, compile and contract/selection checks passed. One local contract invocation lacked PYTHONPATH; the documented `PYTHONPATH=src` invocation passed. Existing Starlette/AnyIO deprecation warnings remain unrelated.

A fresh browser load shows “下一计划已发布 / 目标日K 2026-09-18” and independently “历史日K回补 2026-08-04 / 3043/3044” while that retry was pending. The first previously cached browser page retained old assets; fresh navigation and served asset inspection verified the deployed bundle. Real final chat explicitly distinguished August 4 recovery from September 21 plan / September 18 evidence and confirmed the plan was published, observation-only. Disposable conversations were deleted (204). After native recovery, SHA256 for all 83 prior plans and 4717 prior publications remained identical. Current target stays published, complete 3052/3052 tracked coverage, no approximation. Source [CI 67ce73a](https://github.com/tealigantal/gp/actions/runs/35355433964) passed. This completes the two requested fixes without claiming all historical backlog or any investment-return improvement.
