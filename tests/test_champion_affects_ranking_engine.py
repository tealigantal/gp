...
def test_champion_score_changes_order(monkeypatch):
        ...
        pool = [
            {
                ...
                "liquidity": {"avg5_amount": 2e9, "grade": "A"},
                "chip": {"avg_cost": 10.0, "band_90_low": 9.5, "band_90_high": 10.5},
                "indicators": {"slope20": 0.5, "atr_pct": 0.02, "gap_pct": 0.0},
            },
            {
                "symbol": "000001",
                "industry": "bank",
    ...
    monkeypatch.setattr(
        dh.MarketDataHub,
        "daily_ohlcv",
        lambda self, sym, as_of, min_len=250, prefer_cache_only=False: (_stub_feat_df(), {"len": 260}),
    )
    ...
    monkeypatch.setattr(
        indi,
        "compute_indicators",
        lambda df: df.assign(slope20=0.5, atr_pct=0.02, gap_pct=0.0, ma20=10.0, amount_5d_avg=1e8),
    )
    ...
    def fake_choose(pool):  # noqa: ANN001
        return {
            "600519": {"strategy": "S1", "score": 1.0, "setup_penalty": 0.0, "reasons": []},
            "000001": {"strategy": "S1", "score": 0.1, "setup_penalty": 0.0, "reasons": []},
        }
    ...
    first_component = picks[0].get("score_breakdown", {}).get("champion_component", 0.0)
    second_component = picks[1].get("score_breakdown", {}).get("champion_component", 0.0)
