...
def test_infer_scale_100_when_evidence_present():
    from gp_assistant.recommend.chg_normalize import infer_scale_by_implied_pct, normalize_chg_pct, detect_chg_col
    df = pd.DataFrame({
        '最新价': [10.0, 20.0, 30.0],
        '涨跌额': [0.1, -0.2, 0.3],
        '涨跌幅': [0.01, -0.01, 0.01],  # decimals
    })
    scale = infer_scale_by_implied_pct(df)
    assert scale == 100.0
    chg_col = detect_chg_col(df.columns)
    ser, ev = normalize_chg_pct(df, chg_col or '涨跌幅')
...
def test_infer_scale_1_when_raw_is_percent():
    ...
    df = pd.DataFrame({
        '最新价': [10.0, 20.0, 30.0],
        '涨跌额': [0.1, -0.2, 0.3],
        '涨跌幅': [1.0, -1.0, 1.0],  # already percent
    })
    scale = infer_scale_by_implied_pct(df)
    assert scale == 1.0
    chg_col = detect_chg_col(df.columns)
...
def test_no_evidence_assume_percent_and_record():
    from gp_assistant.recommend.chg_normalize import infer_scale_by_implied_pct, normalize_chg_pct
    df = pd.DataFrame({
        '代码': ['a', 'b', 'c'],
        '涨跌幅': [1.2, -0.3, 0.5],
    })
    ...
    ser, ev = normalize_chg_pct(df, '涨跌幅')
