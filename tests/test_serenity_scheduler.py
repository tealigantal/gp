...
def test_schedule_uses_elapsed_and_phase_floor():
    ...
    assert fast.stale_after_sec > fast.delay_sec + fast.cost_sec
    assert slow.stale_after_sec > slow.delay_sec + slow.cost_sec
...
def test_schedule_backoff_and_circuit_breaker():
    ...
    assert failed.stale_after_sec > failed.delay_sec
    assert limited.stale_after_sec > limited.delay_sec
    assert circuit.stale_after_sec > circuit.delay_sec
