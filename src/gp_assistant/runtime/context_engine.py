...
def _pick_plan_slice(entry) -> Dict[str, Any]:
    ...
    return {
        ...
        "signal_type": getattr(entry, "signal_type", None),
        "signal": getattr(pick, "signal", {}) if pick else {},
        "probability": getattr(pick, "probability", {}) if pick else {},
        "risk": getattr(pick, "risk", {}) if pick else {},
        "ranking": getattr(pick, "ranking", {}) if pick else {},
        "historical_cases": getattr(pick, "historical_cases", []) if pick else [],
        "decision_context_snapshot_id": getattr(pick, "decision_context_snapshot_id", None) if pick else None,
