...
def summarize(trades: pd.DataFrame, daily_equity: pd.DataFrame, fail_counts: Dict[str, int]) -> TradeStats:
    if trades.empty:
        return TradeStats(
            ...
            0.0,
            0,
            fail_counts.get("buy_fail", 0),
            fail_counts.get("sell_fail", 0),
            fail_counts.get("pending_bar_count", 0),
            fail_counts.get("reject_buy", 0),
            fail_counts.get("reject_sell", 0),
        )
