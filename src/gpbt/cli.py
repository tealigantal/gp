...
def main() -> None:
    parser = argparse.ArgumentParser(description='gpbt CLI')
    parser.add_argument('--config', default='configs/config.yaml', help='config file path')

    sub = parser.add_subparsers(dest='cmd', required=True)

    p_init = sub.add_parser('init', help='init workspace directories')

    p_fetch = sub.add_parser('fetch', help='fetch data: stock_basic, calendar, daily, minutes')
    p_fetch.add_argument('--start', required=True, help='YYYYMMDD')
    p_fetch.add_argument('--end', required=True, help='YYYYMMDD')
    p_fetch.add_argument('--max-codes', type=int, default=None, help='limit number of codes to fetch')
    p_fetch.add_argument('--no-minutes', action='store_true', help='skip minute bars')
    p_fetch.add_argument('--max-days', type=int, default=None, help='limit number of trading days for minutes')
    p_fetch.add_argument('--retries', type=int, default=0, help='retries for minute fetch (default 0)')
    p_fetch.add_argument('--min-provider', choices=['eastmoney_curl','akshare','tushare','local_files'], default=None, help='override minute provider')
    p_fetch.add_argument('--codes', type=str, default=None, help='comma-separated ts_code list')

    p_cand = sub.add_parser('build-candidates', help='build candidate pool for a day')
    p_cand.add_argument('--date', required=True, help='YYYYMMDD')

    p_cand_r = sub.add_parser('build-candidates-range', help='build candidates for date range')
    p_cand_r.add_argument('--start', required=True, help='YYYYMMDD')
    p_cand_r.add_argument('--end', required=True, help='YYYYMMDD')

    p_minp = sub.add_parser('fetch-min5-for-pool', help='fetch 5min for candidate pool on date')
    ...
    p_bt = sub.add_parser('backtest', help='weekly backtest')
    ...
    p_bt.add_argument('--require-trades', action='store_true', help='fail if any strategy has zero trades')

    p_doc = sub.add_parser('doctor', help='data and config doctor')
    p_doc.add_argument('--start', required=True, help='YYYYMMDD')
    p_doc.add_argument('--end', required=True, help='YYYYMMDD')

    p_llm = sub.add_parser('llm-rank', help='LLM ranking (JSON cache; supports mock/fallback)')
    p_llm.add_argument('--date', required=True, help='YYYYMMDD')
    p_llm.add_argument('--template', required=True, help='template ID (e.g., momentum_v1)')
    p_llm.add_argument('--force', action='store_true', help='force refresh cached outputs')
    p_llm.add_argument('--topk', type=int, default=3, help='返回TopK（默认3）')

    p_tune = sub.add_parser('tune', help='tune best strategy combo and write current_policy')
    ...
    p_tune.add_argument('--templates', type=str, required=True, help='comma-separated template IDs')
    p_tune.add_argument('--entries', type=str, default='baseline', help='comma-separated entry IDs (default baseline)')
    p_tune.add_argument('--exits', type=str, default='next_day_time_exit', help='comma-separated exit IDs (default next_day_time_exit)')
    ...
    p_llmrun = sub.add_parser('llm-run', help='execute backtest using current_policy + LLM ranking')
    ...
    p_minr = sub.add_parser('fetch-min5-range', help='fetch 5min for candidate pool across date range')
